...
global product
product =0
f = open(u'F:\\GitRespo\\ShopSpider\\Auto\\DataWare\\JDDetail\\$$query$$.csv','w')
f.truncate()
...
class JdOnepage(CrawlSpider):
    ...
    def parse(self, response): 
        ...
        f = open(u"F:\\GitRespo\\ShopSpider\\Auto\\DataWare\\JDTotal\\$$query$$.csv", "r")
        if(TMS<len(f.readlines())):
            line = linecache.getline(u"F:\\GitRespo\\ShopSpider\\Auto\\DataWare\\JDTotal\\$$query$$.csv",TMS+1)
            price = 0
            product_id=0
            URL=0 
            product = line.split(',')
